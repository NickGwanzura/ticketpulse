...
export default function WhatsAppWidget({
  ...
  return (
    <a
      ...
      aria-label={label}
      className="group fixed bottom-[var(--tp-cookie-top,5rem)] right-3 z-50 inline-flex items-center gap-2 rounded-full bg-whatsapp p-2.5 text-on-whatsapp shadow-[0_18px_40px_-12px_rgba(37,211,102,0.55)] ring-1 ring-black/10 transition-[transform,background-color,bottom] hover:scale-[1.03] hover:bg-whatsapp-hover active:scale-[0.99] sm:bottom-[var(--tp-cookie-top,1.75rem)] sm:right-7 sm:gap-2.5 sm:py-3 sm:pl-3.5 sm:pr-4"
    >
      <span className="relative inline-flex h-6 w-6 items-center justify-center rounded-full bg-black/10 sm:h-7 sm:w-7">
        <span className="absolute inset-0 rounded-full bg-black/15 animate-ping opacity-60" aria-hidden />
      ...
      <span className="hidden pr-0.5 text-[12px] font-semibold tracking-tight sm:inline sm:text-[13px]">{label}</span>
    </a>
  )
}
