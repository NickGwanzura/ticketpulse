...
export type PayoutRequestResult = { ok: boolean; message: string; payoutId?: string }

const fail = (message: string): PayoutRequestResult => ({ ok: false, message })

export const ACTIVE_PAYOUT_STATUSES = ["pending", "approved", "processing"] as const
export const VALID_METHODS = ["ecocash", "bank_usd"] as const
...
/** Payout balance for [userId]; zero for accounts that cannot receive payouts. */
export async function fetchBalanceForUser(userId: string) {
...
/**
 * Validate and record an organiser payout request, then email the organiser
 * and admins. Shared by the website action and the organizer mobile API so
 * both apply the same balance, one-active-request, and audit rules.
 * Callers must authenticate [userId] first.
 */
export async function submitPayoutRequest(
  ...
  if (!inserted) {
    ...
    return fail("We could not confirm the payout request. Please try again.")
  }
