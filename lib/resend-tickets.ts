...
/**
 * Resend an order's confirmation email with its ticket link.
 *
 * `organizerScoped` may only be set by server code that has already enforced
 * an organizer+event-scoped check (the organizer mobile API). It is never
 * read from the request, so a client cannot claim it.
 */
export async function resendOrderTickets(req: Request, id: string, opts: { organizerScoped: boolean }): Promise<NextResponse> {
  ...
  const access = await authorizeOrderAccess(id, { ...creds, organizerScoped: opts.organizerScoped })
  if (!access.ok) {
