    ...
    <form action={formAction} className="inline-flex items-center gap-1.5">
      <div className="relative">
        <input
          ...
          min="0"
          max="100"
          className="w-20 rounded-lg border border-line bg-paper px-2.5 py-1.5 text-[12px] text-ink tabular-nums text-right focus:outline-none focus:border-blue focus:ring-4 focus:ring-blue/10 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
        />
      ...
      <button
        type="submit"
        disabled={pending}
        className="rounded-md px-2 py-1.5 text-[11px] font-medium text-blue hover:bg-blue-50 disabled:opacity-40 transition-colors"
      >
        {pending ? "Saving…" : "Save"}
      </button>
      {state && !state.ok && (
