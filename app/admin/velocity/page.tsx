...
export default async function AdminVelocityPage({
  ...
  // ── Extract Velocity metadata and enrich rows ──────────────────────────
  type EnrichedRow = VelocityRow & {
    velocity: VelocityOrderMetadata | null
    hasVelocity: boolean
  }

  const enriched: EnrichedRow[] = orderRows.map((o) => {
    const meta = (o.metadata ?? {}) as { velocity?: VelocityOrderMetadata }
    return {
      ...o,
      velocity: meta.velocity ?? null,
      hasVelocity: !!meta.velocity,
    }
  })
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: allVelocityOrders.length.toLocaleString(),
      icon: Activity,
      tone: "text-navy",
      bg: "bg-navy/5",
    },
    {
      label: "Completed",
      value: velocityPaid.length.toLocaleString(),
      icon: CheckCircle,
      tone: "text-green-600",
      bg: "bg-green-50",
    },
    {
      ...
      value: velocityPending.length.toLocaleString(),
      icon: ShoppingCart,
      tone: "text-amber-700",
      bg: "bg-amber-50",
    },
    {
      ...
      value: velocityFailed.length.toLocaleString(),
      icon: XCircle,
      tone: "text-rose-600",
      bg: "bg-rose-50",
    },
  ...
  const pollIcon = (pollStatus: VelocityPollStatus | string | null | undefined) => {
      ...
      <div className="px-5 md:px-8 py-8 md:py-10 space-y-6">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar">
            {FILTER_PILLS.map(({ label, value }) => {
              ...
              const href = params.toString()
                ? `/admin/velocity?${params.toString()}`
                : "/admin/velocity"
              return (
                <Link
                  key={value}
        ...
        <div className="rounded-2xl border border-line bg-paper overflow-hidden tp-fade-up-3">
          {enriched.length > 0 ? (
            ...
            <EmptyState
              icon={Activity}
              title={query ? "No matching Velocity transactions" : "No Velocity transactions yet"}
              body={
                query
