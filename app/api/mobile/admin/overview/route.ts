...
import { db } from "@/db"
import { events, orders, payouts, supportCases, users } from "@/db/schema"
...
export async function GET(request: Request) {
  ...
  const [eventCounts, liveEvents, supportCaseRows, orderStats, paidValues, payoutRows, organizerRows, recentOrders] = await Promise.all([
    ...
    db.select({ count: sql<number>`count(*)::int` }).from(supportCases).where(eq(supportCases.status, "open")),
  ...
  return NextResponse.json({ ok: true,
    ...
    openSupportCaseCount: Number(supportCaseRows[0]?.count ?? 0),
