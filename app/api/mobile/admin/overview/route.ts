...
export async function GET(request: Request) {
  ...
  const [eventCounts, orderStats, payoutRows, organizerRows, recentOrders] = await Promise.all([
  ...
  return NextResponse.json({ ok: true,
    ...
    recentOrders: recentOrders.map(row => ({ ...row, totalAmount: Number(row.totalAmount), currency: row.currency ?? "USD", buyerName: row.buyerName ?? row.guestName, buyerEmail: row.buyerEmail ?? row.guestEmail })),
